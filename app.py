...
def call_llm_api(user_query: str, context: str) -> str:
    ...
    # openai.api_key = "YOUR_API_KEY"  # or use st.secrets["OPENAI_API_KEY"]
    ...
    st.session_state.mindmap = {
        "nodes": {
            root_id: {
                "id": root_id,
                "title": "Root Topic",
                "content": [],  # list of (user_message, assistant_response) or just strings
                "parent": None,
                "children": [],
                "branch_name": "Root",
            }
...
def add_chat_message(user_query: str):
    ...
    # For a real app, you might also incorporate parent branches, etc.
    # Here, we’ll just join all user+assistant messages from this node:
    full_context = ""
    for turn in active_node["content"]:
        if isinstance(turn, dict):
            full_context += f"User: {turn['user']}\nAssistant: {turn['assistant']}\n"
        else:
            # If it's not in a dict form, we skip or handle differently
            pass
...
def render_mindmap():
    ...
    for node_id, node_info in st.session_state.mindmap["nodes"].items():
        # Label can show the branch_name or the first few words, etc.
        label = node_info["branch_name"]
        # If it's the active node, highlight it
...
def main():
    ...
    if active_node["content"]:
        for turn_idx, turn in enumerate(active_node["content"]):
    ...
    # -- Provide an input for new user queries
    ...
    if "show_mindmap" in st.session_state and st.session_state.show_mindmap:
